...
router.get('/web-browser-extensions', authenticateToken, async (_req, res) => {
  try {
...
router.get('/web-browser-extension/:filename', authenticateToken, (req, res) => {
  const filename = Array.isArray(req.params.filename)
    ? req.params.filename[0]
    : req.params.filename;
  const resourcesPath = process.env.PATH_PROJECT_RESOURCES;
  if (!resourcesPath) {
    return res
      .status(500)
      .json({ result: false, message: 'PATH_PROJECT_RESOURCES is not configured.' });
  }
