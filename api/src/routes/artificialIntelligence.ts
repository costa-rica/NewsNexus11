...
router.post("/add-entity", authenticateToken, async (req, res) => {
  ...
  const ai = await ArtificialIntelligence.create({
    ...
    huggingFaceModelName:
      typeof huggingFaceModelName === "string" ? huggingFaceModelName : null,
    huggingFaceModelType:
      typeof huggingFaceModelType === "string" ? huggingFaceModelType : null,
  });
  ...
  async (_req, res) => {
    ...
    const articlesArrayModified = articlesArray.map((article: any) => {
      ...
      if (article.description === null || article.description === "") {
        const articleApproved = (
          article.ArticleApproveds as (typeof ArticleApproved)[]
        )?.[0] as any;
        if (articleApproved) {
