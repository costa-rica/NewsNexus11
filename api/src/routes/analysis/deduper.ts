...
router.post("/report-checker-table", authenticateToken, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const articleIdToRefNumberMap: Record<string, string> = {};
    ...
    const reportArticleDictionary: Record<string, any> = {};
    ...
    for (const contract of articleReportContracts) {
      ...
      let maxEmbedding = 0;
      const approvedArticlesArray: any[] = [];
...
router.get("/request-job/:reportId", authenticateToken, async (req: Request, res: Response) => {
...
router.get("/job-list-status", authenticateToken, async (_req: Request, res: Response) => {
