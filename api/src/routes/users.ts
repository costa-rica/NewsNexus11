...
import { EntityWhoFoundArticle, User } from 'newsnexus10db';
...
router.post('/request-password-reset', passwordResetLimiter, async (req, res) => {
  ...
  try {
    ...
    const secret = process.env.JWT_SECRET;
    if (!secret) {
      return res.status(500).json({ result: false, error: 'JWT_SECRET is not configured' });
    }
    ...
    logger.error(`❌ [ROUTE DEBUG] Error in /request-password-reset for ${email}:`, {
...
router.post('/reset-password/:token', async (req, res) => {
  ...
  try {
    ...
    if (error.name === 'JsonWebTokenError') {
      return res.status(400).json({ result: false, error: 'Invalid reset token' });
    }
