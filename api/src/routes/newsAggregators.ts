...
router.post("/requests", authenticateToken, async (req, res) => {
  ...
  const newsApiRequestsArray = Array.from(requestsMap.values()).map(
    (r: any) => ({
      ...r,
      includeString: r.includeSourcesArray
        .map((d: { name: string }) => d.name)
        .join(", "),
      excludeString: r.excludeSourcesArray
        .map((d: { name: string }) => d.name)
        .join(", "),
    }),
  );
...
router.post(
  ...
  async (req, res) => {
    ...
    const newsArticleAggregatorSource =
      await NewsArticleAggregatorSource.findByPk(idParam);
    ...
    if (Object.keys(updatedFields).length > 0) {
      ...
      logger.info(
        `News article aggregator source ${idParam} updated successfully`,
      );
    } else {
      logger.info(
        `No updates applied for news article aggregator source ${idParam}`,
      );
    }
...
router.delete(
  ...
  async (req, res) => {
    ...
    logger.info(
      `News article aggregator source ${idParam} deleted successfully`,
    );
