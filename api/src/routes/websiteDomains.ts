...
import { WebsiteDomain } from 'newsnexus10db';
...
router.post('/get-website-domains-array', authenticateToken, async (req, res) => {
  const body = req.body as WebsiteDomainsArrayBody;
  const excludeArchievedNewsDataIo = body.excludeArchievedNewsDataIo === true;

  logger.info('---> excludeArchievedNewsDataIo: ', excludeArchievedNewsDataIo);
