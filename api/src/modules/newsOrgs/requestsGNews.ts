...
export async function makeGNewsRequest(
  ...
  if (!startDate) {
    startDate = new Date(
      new Date().setDate(
        new Date().getDate() - Number(process.env.COUNT_OF_DAYS_HISTORY_LIMIT),
      ),
    )
      .toISOString()
      .replace(".000", "");
...
export async function storeGNewsArticles(
  requestResponseData: any,
  newsApiRequestObj: any,
) {
  ...
  const entityWhoFoundArticleId = (gNewsSource as any)?.EntityWhoFoundArticle
    ?.id;
  try {
...
export async function makeGNewsApiRequestDetailed(
  ...
  if (process.env.ACTIVATE_API_REQUESTS_TO_OUTSIDE_SOURCES === "true") {
    ...
    const normalizedArticles =
      normalizeGNewsArticlesPayload(requestResponseData);
