...
describe('users routes', () => {
  ...
  test('POST /users/register creates user and returns token', async () => {
    ...
    expect(mockEntityWhoFoundArticleModel.create).toHaveBeenCalledWith({ userId: 1 });
  });

  test('POST /users/register rejects duplicate email', async () => {
    mockUserModel.findOne.mockResolvedValue({ id: 99, email: 'dupe@example.com' });
  ...
  test('POST /users/request-password-reset returns success for known user', async () => {
    mockUserModel.findOne.mockResolvedValue({ id: 5, email: 'known@example.com' });
