...
- [ ] Confirm the request and response contract for a new API endpoint that returns article content for the review-page modal.
- [ ] Confirm the request and response contract for a new API endpoint that queues a one-off AI approver run for one article and one newly created prompt row.
- [ ] Confirm the request and response contract for a new worker-python route dedicated to this one-off flow.
- [ ] Confirm the portal payload fields for one-off submission:
  - `articleId`
  - `name`
  - `promptInMarkdown`
  - `sourcePromptVersionId` nullable
- [ ] Confirm the API-generated prompt fields for one-off prompt creation:
...
- [ ] Confirm the content-loading rule:
...
- [ ] Confirm that the review-table icon appears only when at least one `ArticleContents02` row exists for that article.
- [ ] Confirm that the new score row should appear in existing top-score and AI Approver details flows without changing existing score semantics.
- [ ] Update this TODO if the agreed contract changes before implementation starts.
...
- [ ] Commit with a message that references `docs/REVIEW_PAGE_AI_APPROVER_FLOW_20260323_TODO.md` and phase 1.
